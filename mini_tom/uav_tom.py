...
#                                                    self.isovist, 10, 8 )
                ...
                intruder_path = run_rrt( a2d(rrt_loc(IntruderStart)), a2d(rrt_loc(IntruderGoal)), self.X1, self.Y1, self.X2, self.Y2 )
...
#        results.append( rval )
        ...
        try: 
                rval = roll_out( K, isovist, X1, Y1, X2, Y2 )
                print "Detection: ", rval[1]
                np.save( './results_%d_%d_%d.npy' % (os.getpid(),K,len(rval)), rval )
#                results.append( rval )
